(function(window){
    ...
    Seed = function(tree, point, scale, color) {
        ...
        this.textScale = (tree.opt.seed && tree.opt.seed.textScale) || 1;
        this.seedText  = (tree.opt.seed && tree.opt.seed.text)      || 'Dale Click, Amor';
    }
    Seed.prototype = {
        draw: function() {
            this.drawHeart();
        ...
        drawText: function() {
            ...
            ctx.moveTo(0, 0);
            ctx.scale(0.75, 0.75);
            ctx.font = "12px Roboto,Verdana"; 
            ctx.fillText(this.seedText, 23, 10);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
            var point = cirle.point;
            var baseScale = Math.max(cirle.scale || 1, this.heart.scale || 1);
            var textScale = this.textScale || 1;
            // Clear a larger area so enlarged mobile helper text is fully removed.
            var clearSize = 90 * baseScale * Math.max(1, textScale * 0.9);
            ctx.clearRect(point.x - clearSize * 1.3, point.y - clearSize * 1.3, clearSize * 3.8, clearSize * 3.2);
        },
        hover: function(x, y) {
    ...
    Tree.prototype = {
        ...
        initBloom: function() {
            ...
            var cache = [],
                ...
                radius = bloom.radius || 240,
                startScale = bloom.startScale || 0.1,
                figure = this.seed.heart.figure;
            for (var i = 0; i < num; i++) {
                cache.push(this.createBloom(width, height, radius, figure, null, null, null, startScale));
            }
            this.blooms = [];
        ...
        createBloom: function(width, height, radius, figure, color, alpha, angle, scale, place, speed) {
            ...
            var margin = bloomOpt.margin || 20;
            var centerX = width / 2 + (bloomOpt.centerXOffset || 0);
            var centerY = height - (height - 40) / 2 + (bloomOpt.centerYOffset || 0);
            var x, y;
            while (true) {
                x = random(margin, width - margin);
                y = random(margin, height - margin);
                if (inheart(x - centerX, centerY - y, radius)) {
        ...
        jump: function() {
            ...
            if ((blooms.length && blooms.length < 3) || !blooms.length) {
                ...
                for (var i = 0; i < random(1,2); i++) {
                    blooms.push(this.createBloom(width / 2 + width, height, radius, figure, null, 1, null, 1, new Point(random(-100,600), 720), random(200,300)));
                }
            }
        }
    }
    ...
    Branch.prototype = {
        ...
        draw: function(p) {
            ...
            ctx.save();
        	ctx.beginPath();
        	ctx.fillStyle = 'rgb(139, 69, 19)'; // Color marrón para el árbol (saddle brown)
ctx.shadowColor = 'rgb(139, 69, 19)'; 
    ...
    Bloom.prototype = {
        ...
        flower: function() {
            var s = this;
            var bloomOpt = s.tree.opt.bloom || {};
            var growth = bloomOpt.growth || 0.1;
            var maxScale = bloomOpt.maxScale || 1;
            s.draw();
            s.scale += growth;
            if (s.scale > maxScale) {
